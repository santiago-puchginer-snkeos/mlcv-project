...
def train():
    ...
    for ds in dense_sampling_density:
        ...
        for dim_red in pca_reduction:
            ...
            for k in codebook_size:
                ...
                io.save_object(cv_results, 'intersection_svm_optimization_fisher_vectors_l2', ignore=True)
    ...
    io.save_object(best_params, 'best_params_intersection_svm_optimization_fisher_vectors_l2', ignore=True)
    best_params_file = os.path.abspath('./ignore/best_params_intersection_svm_optimization_fisher_vectors_l2.pickle')
    print('Saved at {}'.format(best_params_file))

    print('\nSaving all cross-validation values...')
    io.save_object(cv_results, 'intersection_svm_optimization_fisher_vectors_l2', ignore=True)
    cv_results_file = os.path.abspath('./ignore/intersection_svm_optimization_fisher_vectors_l2.pickle')
    print('Saved at {}'.format(cv_results_file))


def plot_curve():
    print('Loading cross-validation values...')
    cv_values = io.load_object('intersection_svm_optimization_fisher_vectors_l2', ignore=True)

    print('Loading best parameters...')
    best_params = io.load_object('best_params_intersection_svm_optimization_fisher_vectors_l2', ignore=True)
