...
def train():
    ...
    for ds in dense_sampling_density:
        ...
        for dim_red in pca_reduction:
            ...
            for k in codebook_size:
                ...
                fisher, labels = bovw.fisher_vectors(D_pca, L, I, gmm, normalization='l2', spatial_pyramid=True)
                ...
                io.save_object(cv_results, 'pyramid_svm_optimization_fisher_vectors_l2', ignore=True)
    ...
    io.save_object(best_params, 'best_params_pyramid_svm_optimization_fisher_vectors_l2', ignore=True)
    best_params_file = os.path.abspath('./ignore/best_params_pyramid_svm_optimization_fisher_vectors_l2.pickle')
    io.log('Saved at {}'.format(best_params_file))

    io.log('\nSaving all cross-validation values...')
    io.save_object(cv_results, 'pyramid_svm_optimization_fisher_vectors_l2', ignore=True)
    cv_results_file = os.path.abspath('./ignore/pyramid_svm_optimization_fisher_vectors_l2.pickle')
...
def plot_curve():
    ...
    cv_values = io.load_object('pyramid_svm_optimization_fisher_vectors_l2', ignore=True)

    io.log('Loading best parameters...')
    best_params = io.load_object('best_params_pyramid_svm_optimization_fisher_vectors_l2', ignore=True)
    ...
    io.log('\tDense sampling: {:.2f} '.format(best_ds))
    io.log('\tPCA: {:.2f} '.format(best_pca))
    # Subplot parameters
