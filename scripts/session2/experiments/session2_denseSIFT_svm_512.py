...
if __name__ == '__main__':
    ...
    if calculate_results==1:
        ...
        lin_svm, std_scaler, pca = classification.train_rbf_svm(vis_words, labels, C=1.88, gamma=0.0021494, dim_reduction=None)
        print('Elapsed time: {:.2f} s'.format(time.time() - temp))
        temp = time.time()
    ...
    if store_results==1:
        ...
        io.save_object(test_results, 'svm_denseSIFT_test_results_512')
        io.save_object(classes,'svm_denseSIFT_classes_512')
    if plot_results==1:

        print('Loading results object...')
        test_results = io.load_object('svm_denseSIFT_test_results_512', ignore=True)
        classes = io.load_object('svm_denseSIFT_classes_512', ignore=True)
        pred_results = [x[0] for x in test_results]
        pred_class = [x[1] for x in test_results]
        pred_prob = [x[2] for x in test_results]
